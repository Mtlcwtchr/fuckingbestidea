...
namespace Core.Controllers.Core.Controllers
{
    public partial class ControllerBase
    {
        ...
        protected async UniTask ExecuteAndWaitResultAsync<T>(
            ...
            where T : class, IControllerWithResult
        {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                await controller.FlowAsync(cancellationToken);
                cancellationToken.ThrowIfCancellationRequested();

                var result = await controller.GetResult(cancellationToken); //.ConfigureAwait(true);
                cancellationToken.ThrowIfCancellationRequested();
